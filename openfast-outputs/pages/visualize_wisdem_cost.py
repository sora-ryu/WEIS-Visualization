...
def draw_cost_table(cost_options):
    ...
    # fig = go.Figure(data=[go.Table(header=dict(values=['Component', 'Cost']),
    #                                 cells=dict(values=[list(cost_options.keys()), list(cost_options.values())]))])
...
def draw_cost_chart(cost_options):
    ...
    # Main chart - rotor, nacelle, tower
    # fig = go.Figure(data=[go.Pie(labels=[row[0] for i, row in enumerate(cost_matrix) if i != 0], values=[row[1] for i, row in enumerate(cost_matrix) if i != 0], hole=.3)])
    ...
    fig.update_layout(margin = dict(t=0, l=0, r=0, b=0), autosize=False, height=800, width=800,)
